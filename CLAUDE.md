...
当前处于 **0.9.0 已完成——进入 0.9.1 AI 落地阶段**：0.1~0.7 全部完成；0.8.0 ~ 0.8.8 全部落地；**0.9.0 依赖前置 + Action trait tool-call 进化完成**（纯架构零 AI：升 tauri 2.11.5 + reqwest 0.13 + rig-core 0.39,新增 `ActionSchema` + `DangerClass`,12 内置动作 + 3 Chord 全部显式 `schema()` + `danger_class()`,`ActionContext.arguments` 结构化 + `_legacy_arg` 兼容层,SLO tracing target `blink::ai::slo` 就位）。**测试基线 327 通过,零 warning**。AI 相关能力（Provider / 语音 / 生态）规划在 0.9.1 ~ 0.11 三步走（详见 [phases/0.9-ai-layer.md](docs/production-design/phases/0.9-ai-layer.md)）。
...
- 🔜 **0.9.1** Provider 抽象（LLM + STT 接口预留,全 buildin 走 rig）+ 密钥安全（Credential Manager / DPAPI）+ 配置页三档 + `AIConfig` 分片（走 §4.6 类型收窄:主窗口拿到的 `AIProvider` trait 编译期没有 `AgentBuilder` 能力）
- 🔜 **0.9.2** 主窗口文本闭环 + 轻量路由 + 反馈动画 + SLO 埋点上数据 + 未命中过滤四筛子 spike 阈值。详见 [phases/0.9-ai-layer.md](docs/production-design/phases/0.9-ai-layer.md)
