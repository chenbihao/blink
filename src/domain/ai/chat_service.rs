...
impl ChatService {
        ...
        let task = tokio::spawn(async move {
                ...
                .await
            {
                Ok(provider) => {
                    ...
                    let summary_usage_percent = context_status.usage_percent;
                    // 从 persistent_memory 的配置句柄读取 summary_enabled 标志
                    let summary_enabled = {
                        let mem_cfg = service.persistent_memory.config_handle();
                        let mem_cfg_guard = mem_cfg.read().await;
                        mem_cfg_guard.summary_enabled
                    };
    ...
    /// 不是正确性依赖。
    ///
    /// **LLM 调用**：复用缓存的 AgentProvider，用唯一 conversation ID
    ...
    pub async fn maybe_spawn_summary_task(
        ...
        let orphan_ids = [
            format!("__summary__{conversation_id}"),
            format!("__merge__{conversation_id}"),
        ];
        ...
        let _ = async {
            ...
            // D5: 思考强制关，max_tokens 600
            let summary_conv_id = format!("__summary__{conversation_id}");
            let (summary_tx, mut summary_rx) = mpsc::unbounded_channel::<ChatStreamChunk>();
            let summary_prompt = prompt.clone();
            let provider_clone = provider.clone();
            let conv_id_for_llm = summary_conv_id.clone();
            let conv_id_for_error = summary_conv_id.clone();

            // 30s 超时——摘要是后台优化，不能无限制等待
            let summary_timeout = std::time::Duration::from_secs(30);

            let llm_task = tokio::spawn(async move {
                provider_clone.stream_prompt_with_max_tokens(
                    &conv_id_for_llm, &summary_prompt, summary_tx, false, None,
            ...
            let llm_result = tokio::time::timeout(summary_timeout, async {
                while let Some(chunk) = summary_rx.recv().await {
                    match chunk {
            ...
            if summary_text.is_empty() {
                tracing::warn!(conversation_id, "摘要任务: LLM 返回空摘要，跳过");
                return;
            }
    ...
    async fn maybe_merge_summaries(
        ...
        let provider_clone = provider.clone();
        let conv_id_for_llm = merge_conv_id.clone();
        let conv_id_for_error = merge_conv_id.clone();
        ...
        let merge_result = tokio::time::timeout(std::time::Duration::from_secs(30), async {
            while let Some(chunk) = merge_rx.recv().await {
                match chunk {
                    ChatStreamChunk::Text { text } => {
        ...
        if merge_result.is_err() {
            tracing::warn!(conversation_id, "段合并: LLM 超时，跳过");
            return;
        }

        let merged_text = merged_text.trim();
        if merged_text.is_empty() {
            tracing::warn!(conversation_id, "段合并: LLM 返回空，跳过");
            return;
        }
...
struct SummaryInFlightGuard<'a>(&'a std::sync::atomic::AtomicBool);
...
mod tests {
    ...
    fn summary_in_flight_guard_releases_on_early_return() {
        ...
        fn inner(flag: &std::sync::atomic::AtomicBool) -> Result<(), &'static str> {
            let _guard = SummaryInFlightGuard(flag);
