...
impl EngineManager {
    ...
    /// ParaformerOnline ONNX worker 的 health 验证（Handoff 08）：二进制协议 v2 握手。
    ///
    /// 流程：
    /// 1. 从 `ManagedProcess` 取走 stdin/stdout（worker 协议通道）；
    /// 2. `send_hello` + `wait_ready`——worker 在 ORT Session 与 encoder/decoder
    ///    模型真实加载成功后才回 Ready；EOF/超时归为 Fatal（无 bind race 语义）；
    /// 3. 构造 `ParaformerOnlineAdapter`（持有受管进程句柄）存入 entry，
    ///    供 `get_connection` 投影为 VoiceService 的 StreamingSttPort；
    /// 4. 身份/模型校验不依赖协议回显——stdio 管道与 spawn 的子进程一一绑定
    ///    （进程句柄即身份），模型身份/指纹来自 start 冻结的部署 manifest
    ///    （安装事务已做 SHA-256 校验，worker 端 `validate_deployment` 再次校验）。
    pub(super) async fn verify_paraformer_worker_health(
        &self,
        engine_id: &EngineId,
        entry: &Arc<EngineEntry>,
        identity_input: &ServiceIdentityInput,
        ...
        let _ = identity_input; // 二进制协议无身份回显——进程句柄即身份（见 doc）
        ...
        let client = StreamWorkerClient::new(Box::new(stdio.stdin), Box::new(stdio.stdout));

        // 超时复用 implementation descriptor 覆盖值（无覆盖则退回 engine 默认）：
        // ready 总窗口 = start + model_load（ORT 初始化 + encoder/decoder 加载）。
        let timeouts = self
            .implementation_registry
            .descriptor(ImplementationId::ParaformerOnnxWorker)
            .and_then(|d| d.timeouts.clone())
            .unwrap_or_else(|| entry.adapter.descriptor().timeouts.clone());
        let ready_timeout = timeouts.start_timeout + timeouts.model_load_timeout;

        tracing::info!(
            engine = %engine_id,
            model = %frozen_model.model_id,
            ready_timeout_secs = ready_timeout.as_secs(),
            "paraformer worker: hello + 等待 Ready（模型加载完成后回帧）"
        );

        if let Err(e) = client.send_hello().await {
            return Err(StartAttemptFailure::Fatal(LocalEngineError::with_detail(
                LocalEngineErrorCode::ServiceUnreachable,
                ErrorPhase::Health,
                "worker hello 握手失败",
        ...
        if let Err(e) = client.wait_ready(ready_timeout).await {
            // 附带 worker stderr 日志尾——部署缺文件/ORT 加载失败的原因
            // 由 worker 写到 stderr（LogPipe 泵入 log history）
...
",
            ...
            let code = if client.is_poisoned() {
                // EOF / 协议违例——进程早退或输出污染
...
{tail_text}"
        ...
        // Model 必须 Ready（ready 帧语义 = ORT + 模型加载完成；模型身份/指纹
        // 由 start 冻结的部署 manifest 承载，见方法 doc）。
        let mapping = HealthMapping {
            ...
            backend: Some(crate::domain::local_engine::BackendObservation {
                actual_backend: crate::infra::local_engine::runtime::ComputeBackend::Cpu,
                device_name: "CPU (ONNX Runtime)".to_string(),
                consistent: true,
            }),
            model_id: Some(frozen_model.model_id.clone()),
            model_revision: Some(frozen_model.revision.clone()),
            model_content_fingerprint: frozen_model.fingerprint.clone(),
        };

        // 存入 entry——get_connection 据此投影 StreamingSttPort；
        // 适配器持有受管进程句柄，stop/exit 时随 entry 字段清理。
        {
            let mut sp = entry.streaming_port.lock().await;
            *sp = Some(std::sync::Arc::new(
                crate::infra::local_engine::streaming_stt_adapter::ParaformerOnlineAdapter::with_process(
                    client,
                    Arc::clone(managed),
                ),
            ));
