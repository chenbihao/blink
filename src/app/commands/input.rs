...
//! 通过 `update_main_input_context` 上报离散视图上下文变化（query 是否为空 / AI mode）。
...
/// 并发送到 hook 线程更新输入状态机。
#[tauri::command]
pub fn register_main_input_view(query_empty: bool, ai_mode: bool) -> RegisterViewResult {
...
/// 更新主窗口输入视图上下文（query 是否为空 / AI mode 变化时调用）。
///
/// 前端只在 `query_empty` 或 `ai_mode` 实际发生变化时调用，不逐字符发送。
/// 携带 `view_epoch` + 递增 `revision`；旧 epoch 的 update 被后端丢弃。
#[tauri::command]
pub fn update_main_input_context(view_epoch: u64, revision: u64, query_empty: bool, ai_mode: bool) {
    let ctx = MainViewContext {
        view_epoch,
        revision,
        ready: true,
