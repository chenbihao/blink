...
按住 **Alt+Space** 说话，松开自动上屏。长按说话属于短输入，始终投递到开始手势时冻结的光标/选区目标：
...
编辑器另提供与长按说话独立的**连续录音**：由编辑器内按钮显式开始，confirmed 文本持续追加到文档尾部，只提供暂停/继续与结束；失焦、最小化和切换应用不停止。STT 失败或关闭目标窗口时结束录音并保留已确认文字、丢弃 preview。系统记录本次追加范围，录音结束后提供“定位到本次听写”和“整理本次听写”两个显式动作；不自动上传、不自动改写，也不建立整次录音管理器。
...
主窗口输入框在不同输入意图下表现为**三个互斥模式**，各自拥有独立的渲染区和键盘逻辑，避免相互污染。

| 模式 | 触发 | 渲染区 | 输入框 |
|---|---|---|---|
| **搜索模式**（默认） | 默认 / 从其他模式退出 | `#results` 结果列表 | `#query` |
| **AI 模式** | 搜索结果中 AI Ghost + Tab 接受 | `#ai-display` 流式对话区（整块 DOM 切换，搜索区隐藏） | `#ai-query`（独立输入框） |
| **命令模式** | `> ` 前缀自动检测 | 命令 hint（结果区清空，不触发搜索） | `#query`（复用，但搜索逻辑短路） |

**模式隔离铁则**：
- 进入非搜索模式后，搜索/ghost/结果区**完全切支**--不触发 `search_apps`、不显示 ghost、回车不走默认搜索项执行
- 退出则完整恢复搜索模式（清空输入、复位焦点、重置窗口尺寸）
- 三模式互斥：AI 模式走独立 DOM 块 + 独立输入框，命令模式入口在搜索 `onInput` 最前置--结构上保证不会同时处于两个非搜索模式

**退出路径**：
- AI 模式：`ESC` 退回搜索模式（不 hide 窗口）；中止活跃请求、清看门狗标志、复位窗口尺寸
- 命令模式：退格删掉 `>` 前缀即自动退出（不需要 ESC）；`ESC` 则直接隐藏窗口
- 窗口隐藏/重新唤起时 lifecycle 复位所有模式，每次从搜索模式干净起步

> 这是 Blink 对"输入框多意图"的设计回答：不混在一个渲染区里靠条件分支切换，而是模式级隔离 + ESC 回退到默认搜索。命令模式（0.18.6）是首个"用户主动声明意图模式"，与 AI 模式（0.9+）的"后端产 Ghost 建议用户接受"形成互补。
...
**定位**：编辑器是“捕获 → 整理 → 交付”的临时文本任务会话，不是笔记库、IDE 或内嵌 Chat。它把剪贴板、选区、便签、OCR/翻译等能力结果和空白/语音输入接入同一工作台，经人工、语音或 AI 整理后，明确交付到剪贴板、原便签、调用方或用户确认的文件。

**会话与资源模型**：0.23 只保留一个预热编辑器窗口和一个活动 EditorSession，避免窗口池、任务管理和跨窗口竞态稀释核心体验。再次打开同一持久来源时激活现有任务；不同来源的新请求不得覆盖正文，而是提示先完成当前任务。会话结束后窗口完整清理并回到预热态。

**语音边界**：全进程任意时刻最多只有一个活动 VoiceSession，G1/G2/G3/Editor 共同排他。长按说话冻结当前光标/选区，连续录音固定向编辑器文档尾部追加；两者不因焦点或最小化而迁移。

**首版 AI 整理**：编辑器只提供“整理选中内容”和“整理本次听写”，两者都需要用户明确触发，用于修正明显误识别、补删标点、自然分段和缩减无意义口水词，不建立其他改写预设或自定义指令。请求使用快速模型档、关闭思考、无记忆且无工具，并服从主窗口、Chat、Editor 共用的全进程 AI 单活跃约束。结果在非模态底部面板以中文友好的 Diff 确认后再替换；请求后正文发生任何变化，旧候选即不可替换，只能复制或重新整理。

**发送到 AI 对话**：这是与听写整理分离的单向内容流转。有选区时动作明确显示“发送选中内容到 AI 对话”，无选区时显示“发送全文到 AI 对话”；只预填 Chat composer，不自动发送。编辑器保持打开且主保存目标不变，首版不建立 Chat 返回编辑器 patch 的双向会话链路。

**视图由来源声明**：内部用 `disabled / available / preferred` 三态描述来源的 Markdown 视图策略，分别表示仅原文、支持 MD 但默认原文、支持并默认 MD。空白编辑器和便签默认 MD；剪贴板、外部选区和普通能力结果默认原文但可手动切换；明确不适合 Markdown 的来源隐藏 MD。来源策略只决定产品入口，不能绕过内容保真风险门；弱内容检测只建议，不自动切换。

**动作层级**：编辑器采用“主动作常驻，其余收进更多”的单层布局，不增加侧栏、命令面板或多层工具栏。顶部展示任务标题、来源和“原文 / MD”；底部展示当前保存目标、保存和连续录音。选区存在时显示“整理选中内容”，录音结束后显示“定位/整理本次听写”。保存到、另存为副本、复制、创建便签和发送到 AI 对话进入“更多”，不可用动作直接隐藏。

**窗口与键盘**：编辑器是普通任务窗口，失焦不自动隐藏。`Esc` 优先取消 AI 请求或关闭 Diff；连续录音活动时不停止录音、不关闭窗口；其余状态请求关闭当前编辑器并执行 dirty 保护。连续追尾只追加文末，不抢焦点、不移动用户在前文的选区。切换“原文 / MD”会取消整理请求和候选，避免跨编辑引擎维护脆弱的位置映射。

**人的主权**：持久来源同一时刻只允许一个高级编辑会话。便签进入编辑器后，原便签窗口只读并可一键返回编辑器；会话结束或异常退出后释放编辑租约。AI 和调用方回写均不得越过 Diff、revision 与目标有效性检查静默改正文。

**保存与完成语义**：每个会话始终展示真实主保存目标，`Ctrl+S` 保存但不关闭；“保存到…”经用户选择后切换主目标，“另存为副本…”只额外输出且不改变主目标。剪贴板来源和空白会话第一次保存创建一条会话结果历史，后续保存更新同一项，不堆积中间历史；便签和已确认文件目标原位保存。clean 关闭直接结束，dirty 关闭提供保存并关闭、放弃修改和继续编辑。用户主动退出 Blink 且存在 dirty 会话时只确认一次；系统关机或强制退出不阻塞。0.23 不建立磁盘恢复草稿或恢复中心。

**冲突与大文本降级**：文件或便签 revision 变化时不自动合并、不静默覆盖；文件由用户选择覆盖、另存副本或取消，便签由用户选择复制当前内容、重新加载或取消。0.23.0 明确整个编辑器的文档大小 envelope；在该范围内 Source 始终可完整编辑和保存，MD、Diff 或 AI 超过各自阈值时只关闭对应增强入口，不截断正文。
...
**远期文档检索的隐私边界**：RAG / 向量化当前不立项。若未来重新评估，本地文档必须默认不出机；启用云端 embedding 前，必须显式说明发送的文档片段、接收方与关闭/删除方式。详见 [roadmap §五](./roadmap.md#五远期观察原-020-向量--rag-规划归档)。
...
| 单体工具 | Blink |
|---|---|
| 翻译是截图工具栏里的一个按钮 | 翻译是一个**能力**，截图选区、便签、剪贴板、AI 对话都能调它 |
| 图片编辑绑定在截图入口 | 图片编辑是一个**能力**，截图、pin 图、剪贴板图片都能进编辑器 |
| 感知（选区/剪贴板/前台）只服务于搜索建议 | 感知是**能力**，搜索、chord 动作、AI 上下文都能消费它 |
| AI 对话是一个独立窗口 | AI 是**消费者**（调 Blink 的 tool）兼**生产者**（产 Suggestion），它的 skill/CLI 能力反过来也是 Blink 可调度的能力 |

**这是最小操作路径在"多能力"维度的延伸**（§6.2）：不只让单目标的路径变短，还让目标之间不再有断裂的搬运成本--截图里的文字直接翻译、翻译结果直接 pin、pin 住的图直接编辑，无需在单体软件之间复制粘贴。

**未来演进**：能力共生不是静态的"当前这几条流转路径"，而是一个**可组合的能力底座**。新能力加入时，它天然就能被所有现有场景调用、也能调用所有现有能力，不需要为每对组合专门写集成。AI 的 skill/CLI、第三方的插件动作、未来的新感知源，都应作为可被任意场景调度的能力单元接入，而非绑死在某个入口的独立功能。
