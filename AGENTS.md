...
当前 **0.12.0-0.12.4 全部完成**（基础设施 → 对话窗口 → 体验优化 → 体验修复：15 项 bug 修复与优化，含侧边栏切换/模型下拉/语音输入根因修复 / 对话标题与布局优化 / 模型选择器移至输入框 / 工具调用渲染修复 / 消息宽度优化），测试基线 **855 通过**。**0.12.5 规划中**（功能增强：主窗口引导泡泡 / 对话标题 LLM 命名 + 设置优化 / 消息编辑重发 / 导出对话 / 代码块语法高亮）。**0.12.6 规划中**（对话分组 + 分组系统提示词）。详见 [phases/0.12-ai-ecosystem.md](docs/production-design/phases/0.12-ai-ecosystem.md)。
...
- 📋 **0.12.5 功能增强**（规划中）：主窗口功能引导泡泡（空 query 展示示例 prompt，点击预填充）/ 对话标题 LLM 自动生成（设置开关 + 模型档位选择：主档/轻量档/路由档）/ 设置页优化（AI Tab 新增对话配置区）/ 消息编辑重发 / 导出对话为 Markdown / 代码块语法高亮。详见 [phases/0.12-ai-ecosystem.md §五](docs/production-design/phases/0.12-ai-ecosystem.md)
