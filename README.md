...
Blink 的底层是一套**统一能力底座**。便签、图片、剪贴板、截图、OCR、翻译等不再是彼此孤立的功能：一个能力产生的内容，可以直接成为下一个能力的输入，在 Blink 内连续流转，无需反复保存、切换软件和复制粘贴。

这些能力既可以被 Blink 内置 AI 调用，也可以通过本机 **MCP Server** 对外暴露。用户可以自主选择允许暴露的能力，让其他 Agent 调用当前 Blink 主进程中的搜索、插件、窗口、图片与便签能力。

---

## 能力闭环：内容在能力之间自然流转

Blink 的功能不是一座座孤岛。一个能力的产出，可以直接成为另一个能力的输入：

- **图片闭环** — 截图、窗口或剪贴板图片，可以继续标注、OCR、钉图、复制或保存
- **便签闭环** — AI 可以创建、读取、更新和回收便签，让临时内容能够继续处理
- **剪贴板闭环** — 文本与图片既是感知来源，也可以作为其他能力的输入或输出
- **AI 调用** — Blink 内置 AI 可以按需组合这些能力，完成多步骤任务
- **MCP 对外开放** — Blink 可以作为本机 MCP Server，将用户选择的能力提供给其他 Agent

这不是简单地把便签、截图和 AI 放进同一个应用，而是让它们共享同一套能力语义和运行时状态，减少在不同软件之间搬运内容的成本。
...
语音输入：
<p align="center">
  <img src="docs/images/feature-voice.gif" width="680" alt="AI 能力"/>
</p>
...
- **能力底座收敛** — 让更多现有能力共享实现，并形成更自然的组合路径
