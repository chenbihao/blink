...
- 🎤 **语音输入** — 按住 Alt+Space 说话，松开自动上屏；支持云端（OpenAI/Groq）与本地（FunASR）双引擎
...
| 按住 `Alt + Space` | 语音输入（松开上屏） |

### 语音输入

按住 **Alt+Space** 说话，松开后文字自动上屏——不用切输入法、不用打字。

- **两种场景**：主窗口语音输入（文字填入搜索框）和**语音输入法**（直接在当前光标处上屏，相当于用语音打字）
- **双引擎**：云端（OpenAI Whisper / Groq，按音频时长计费）和本地（FunASR SenseVoice，离线免费，CPU 17× 实时，准确率 ~92%）
- **伪流式**：边说边出字，VAD 自动切句定稿，不用等整句说完才出结果
- **隐私**：本地模式完全离线，录音不持久化；云端模式录音发送到供应商
...
**已完成：** 基础搜索 → 插件生态 → 上下文感知 → Chord 交互 → AI 对话能力 → 前端架构重整 → 语音输入
