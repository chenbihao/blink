...
function testSingleScreen100() {
  ...
  // bitmap ↔ CSS（1:1）
  const css1 = bitmapToCss(bitmap1.x, bitmap1.y, meta, dpr);
  assertFloatEqual(css1.x, 100, 'bitmap → CSS x');
  assertFloatEqual(css1.y, 200, 'bitmap → CSS y');
  const bitmap1Back = cssToBitmap(css1.x, css1.y, meta, dpr);
  ...
  const rectCss = rectScreenToCss(rectScreen, meta, dpr);
  assertEqual(rectCss, { x: 50, y: 60, w: 200, h: 100 }, 'rect screen → CSS');
  const rectScreenBack2 = rectCssToScreen(rectCss, meta, dpr);
  assertEqual(rectScreenBack2, rectScreen, 'rect CSS → screen 往返');
}

function testSingleScreen150() {
  console.log('\n=== 单屏 150% (DPR=1.5) ===');
  const meta = { vx: 0, vy: 0 };
  const dpr = 1.5;
  ...
  const screen1 = cssToScreen(css1.x, css1.y, meta, dpr);
  assertEqual(screen1, { x: 150, y: 300 }, 'CSS → screen (150%)');
  const css1Back = screenToCss(screen1.x, screen1.y, meta, dpr);
  assertFloatEqual(css1Back.x, 100, 'screen → CSS 往返 x');
  assertFloatEqual(css1Back.y, 200, 'screen → CSS 往返 y');

  // 矩形转换
  const rectCss = { x: 80, y: 120, w: 200, h: 100 };
  const rectScreen = rectCssToScreen(rectCss, meta, dpr);
  assertEqual(rectScreen, { x: 120, y: 180, w: 300, h: 150 }, 'rect CSS → screen (150%)');
  const rectCssBack = rectScreenToCss(rectScreen, meta, dpr);
  assertFloatEqual(rectCssBack.x, 80, 'rect screen → CSS x');
  assertFloatEqual(rectCssBack.y, 120, 'rect screen → CSS y');
  assertFloatEqual(rectCssBack.w, 200, 'rect screen → CSS w');
  assertFloatEqual(rectCssBack.h, 100, 'rect screen → CSS h');
}

function testLeftNegativeSecondary() {
  console.log('\n=== 左侧负坐标副屏 ===');
  const meta = { vx: -1920, vy: 0 }; // 主屏 1920x1080，左侧副屏
  const dpr = 1;
...
function testTopNegativeSecondary() {
  console.log('\n=== 上方负坐标副屏 ===');
  const meta = { vx: 0, vy: -1080 }; // 主屏 1920x1080，上方副屏
  const dpr = 1;
...
function testCssBitmapRoundTrip() {
  console.log('\n=== CSS ↔ bitmap 往返换算 ===');
  const meta = { vx: -500, vy: -300 }; // 复杂原点
  const dpr = 2;

  // 用整数 CSS 坐标避免两次取整的累积误差
  const css1 = { x: 100, y: 200 };
  const bitmap1 = cssToBitmap(css1.x, css1.y, meta, dpr);
  const css1Back = bitmapToCss(bitmap1.x, bitmap1.y, meta, dpr);
  // 整数 CSS 坐标往返应精确
