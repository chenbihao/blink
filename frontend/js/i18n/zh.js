...
"ai.followup_placeholder": "追问…",
    ...
    "ai.chat.lede": "对话窗口行为、记忆策略、Skill 约定式与 MCP 外部工具的统一配置。",
    ...
    // 0.13.1 记忆策略
    "ai.memory.section": "记忆策略",
    "ai.memory.section.desc": "上下文窗口管理与历史记忆召回",
    ...
    "ai.memory.trigger_ratio.label": "触发压缩阈值（%）",
    "ai.memory.trigger_ratio.hint": "当窗口总 token 占模型上下文窗口的比例超过此值时触发压缩。默认 80%，留 20% 给 system prompt 和新消息",
    "ai.memory.compress_ratio.label": "压缩目标（%）",
    "ai.memory.compress_ratio.hint": "压缩后保留的最大 token 占比。默认 70%，压缩到模型上下文窗口的 70%",
...
"ai.memory.context_size.unknown": "未知",
    // 0.13.2 FTS5 召回
    "ai.memory.recall.label": "历史记忆召回",
    "ai.memory.recall.hint": "开启后，被窗口裁剪的旧消息自动归档到全文索引，新对话时 BM25 检索召回相关旧上下文注入 <memory> 标签。零嵌入模型依赖",
...
// 0.13.3 Skill 约定式
    ...
    "ai.advanced.section.desc": "AI 调用行为与工具链路控制",
    ...
    // ── AI 权限记忆（0.17.8）──
    "ai.permission.section": "权限记忆",
    "ai.permission.section.desc": "危险操作确认的跨会话记忆",
