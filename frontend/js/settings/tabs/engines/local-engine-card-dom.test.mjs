...
function makeEntry(engineId, overrides = {}) {
    ...
    const {status, models, preferences, logs, catalog} = overrides;
    state = setCatalog(state, catalog || makeCatalog());
...
await test("FunASR 单 compute profile：静态文本，无下拉", () => {
    const container = makeContainer();
    const catalog = makeCatalog().map((item) => item.engine_id === "funasr"
        ? {
            ...item,
            compute_options: [item.compute_options[0]],
    ...
    assert.equal(config.querySelector(".le-config-rebuild-hint").hidden, true, "无需重建时不显示提示");
});

await test("FunASR 多候选完全消费 catalog：顺序保留且不兼容项禁用并说明原因", () => {
    const container = makeContainer();
    const catalog = makeCatalog().map((item) => item.engine_id === "funasr"
        ? {
            ...item,
            model_id: "SenseVoice",
            compute_options: [
                {preference: "cpu", profile_id: "cpu", backend: "cpu", compatible: true, disabled_reason: null},
                {preference: "vulkan", profile_id: "vk", backend: "vulkan", compatible: false, disabled_reason: "host_incompatible"},
                {preference: "cuda", profile_id: "cu", backend: "cuda", compatible: true, disabled_reason: null},
            ],
            undeclared_compute_preferences: ["directml"],
            current_compute_preference: "cpu",
        }
        : item);
    const entry = makeEntry("funasr", {
    ...
    renderEngineCard(container, entry, controllerStub, undefined);
    const config = container.querySelector(".le-card-config");
    const select = config.querySelector(".le-compute-select");
    assert.ok(select, "多个已声明候选显示下拉框");
    assert.deepEqual(queryAll(select, "option").map((option) => option.value), ["cpu", "vulkan", "cuda"]);
    assert.equal(queryAll(select, "option")[1].disabled, true, "不兼容候选 disabled");
    assert.equal(queryAll(select, "option")[1].getAttribute("title"), "本机与此后端不兼容。");
    assert.ok(config.textContent.includes("不可用选项"), "显示不可用候选说明");
});

await test("切换到 CPU-only 模型后归一化 compute preference，不残留幽灵值", () => {
    const container = makeContainer();
    const senseCatalog = makeCatalog().map((item) => item.engine_id === "funasr"
        ? {
            ...item,
            model_id: "SenseVoice",
            compute_options: [
                {preference: "cpu", profile_id: "cpu", backend: "cpu", compatible: true},
                {preference: "vulkan", profile_id: "vk", backend: "vulkan", compatible: true},
            ],
            current_compute_preference: "vulkan",
        }
        : item);
    renderEngineCard(container, makeEntry("funasr", {
        catalog: senseCatalog,
        status: READY_STOPPED,
        preferences: makePreferences({compute_preference: "vulkan"}),
    }), controllerStub, undefined);
    assert.ok(container.querySelector(".le-compute-select"), "SenseVoice 有多个候选");

    const nanoCatalog = makeCatalog().map((item) => item.engine_id === "funasr"
        ? {
            ...item,
            model_id: "Paraformer-Nano",
            compute_options: [
                {preference: "cpu", profile_id: "cpu", backend: "cpu", compatible: true},
            ],
            undeclared_compute_preferences: ["vulkan", "cuda"],
            current_compute_preference: "cpu",
        }
        : item);
    const nano = makeEntry("funasr", {
        catalog: nanoCatalog,
        status: READY_STOPPED,
        preferences: makePreferences({compute_preference: "vulkan"}),
    });
    renderEngineCard(container, nano, controllerStub, undefined);
    const config = container.querySelector(".le-card-config");
    assert.equal(queryAll(config, ".le-compute-select").length, 0, "CPU-only 模型无下拉框");
    assert.equal(config.querySelector(".le-compute-static").textContent, "CPU");
    assert.ok(config.textContent.includes("未被当前模型声明"), "显示未声明偏好归一化原因");
});

await test("运行中状态区分 requested、active profile、actual backend 和待应用状态", () => {
    const container = makeContainer();
    const catalog = makeCatalog().map((item) => item.engine_id === "funasr"
        ? {
            ...item,
            compute_options: [
                {preference: "cpu", profile_id: "cpu", backend: "cpu", compatible: true},
                {preference: "cuda", profile_id: "cuda", backend: "cuda", compatible: true},
            ],
            current_compute_preference: "cuda",
        }
        : item);
    const entry = makeEntry("funasr", {
        catalog,
        status: {
            ...RUNNING_READY,
            backend: {
                requested_preference: "cuda",
                resolved_profile: {profile_id: "cuda-profile", backend: "cuda"},
                desired_deployment: {
                    install_id: "cuda-install",
                    dll_artifact_id: "cuda-runtime",
                    model_generation_id: "sensevoice-cuda",
                },
                loaded_deployment: {
                    install_id: "cpu-install",
                    dll_artifact_id: "cpu-runtime",
                    model_generation_id: "sensevoice-cpu",
                },
                ...
                backend_verification: {
                    ...
                    actual_backend: "cpu",
                    mismatch_reason: "health_mismatch",
                },
            },
        },
        preferences: makePreferences({compute_preference: "cuda", requires_rebuild: true}),
    });
    renderEngineCard(container, entry, controllerStub, undefined);
    const config = container.querySelector(".le-card-config");
    const status = config.querySelector(".le-compute-status");
    assert.ok(status.textContent.includes("偏好：CUDA"), status.textContent);
    assert.ok(status.textContent.includes("运行中 profile"), status.textContent);
    assert.ok(status.textContent.includes("实际后端：CPU"), status.textContent);
    assert.ok(status.textContent.includes("重启或重建"), status.textContent);
    assert.ok(config.querySelector(".le-config-rebuild-hint"), "显示重建提示节点");
    assert.ok(config.querySelector(".le-config-restart-hint"), "显示重启提示节点");

    const sameProfile = makeEntry("funasr", {
        catalog,
        status: {
            ...RUNNING_READY,
            backend: {
                requested_preference: "cuda",
                resolved_profile: {profile_id: "cuda-profile", backend: "cuda"},
                desired_deployment: {install_id: "cuda-install", dll_artifact_id: "cuda-runtime"},
                loaded_deployment: {install_id: "cuda-install", dll_artifact_id: "cuda-runtime"},
                pending_restart: false,
                backend_verification: {state: "healthy", expected_backend: "cuda", actual_backend: "cuda"},
            },
        },
        preferences: makePreferences({compute_preference: "cuda", requires_rebuild: false}),
    });
    renderEngineCard(container, sameProfile, controllerStub, undefined);
    assert.ok(!container.querySelector(".le-compute-status").textContent.includes("重启或重建"),
        "desired 与 active 相同且无需重建时不提示待应用");
});

await test("auto 只显示策略，actual backend 分别消费 CPU/GPU 状态", () => {
    const container = makeContainer();
    const catalog = makeCatalog().map((item) => item.engine_id === "funasr"
        ? {
            ...item,
            compute_options: [
                {preference: "auto", profile_id: "auto-cpu", backend: "cpu", compatible: true},
                {preference: "cpu", profile_id: "cpu", backend: "cpu", compatible: true},
                {preference: "cuda", profile_id: "cuda", backend: "cuda", compatible: true},
            ],
        ...
        : item);
    const cpuAuto = makeEntry("funasr", {
        catalog,
        status: {
            ...RUNNING_READY,
            backend: {
                requested_preference: "auto",
                resolved_profile: {profile_id: "auto-cpu", backend: "cpu"},
                fallback_reasons: [{rejected_profile: "cuda", reason: "host_incompatible", detail: "本机不兼容 CUDA"}],
                backend_verification: {state: "healthy", expected_backend: "cpu", actual_backend: "cpu"},
            },
        },
        preferences: makePreferences({compute_preference: "auto"}),
    });
    renderEngineCard(container, cpuAuto, controllerStub, undefined);
    let status = container.querySelector(".le-compute-status");
    assert.ok(status.textContent.includes("偏好：自动"), status.textContent);
    assert.ok(status.textContent.includes("实际后端：CPU"), status.textContent);
    assert.ok(status.textContent.includes("自动回退原因"), status.textContent);

    const gpuAuto = makeEntry("funasr", {
        catalog,
        status: {
            ...RUNNING_READY,
            backend: {
                requested_preference: "auto",
                resolved_profile: {profile_id: "auto-cuda", backend: "cuda"},
                backend_verification: {state: "healthy", expected_backend: "cuda", actual_backend: "cuda"},
            },
        },
        preferences: makePreferences({compute_preference: "auto"}),
    });
    renderEngineCard(container, gpuAuto, controllerStub, undefined);
    status = container.querySelector(".le-compute-status");
    assert.ok(status.textContent.includes("实际后端：CUDA"), status.textContent);
    assert.ok(!status.textContent.includes("不一致"), "auto 实际落 GPU 不误报偏好不一致");
});

await test("显式 GPU 安装失败仍显示错误状态，不乐观声称已生效", () => {
    const container = makeContainer();
    const catalog = makeCatalog().map((item) => item.engine_id === "funasr"
        ? {
            ...item,
    ...
    const entry = makeEntry("funasr", {
        ...
        status: {
            ...READY_STOPPED,
            last_error: {
                code: "compute_profile_install_failed",
                message: "CUDA artifact self-test failed",
                action_hint: "Keep the current CPU profile and retry after fixing the GPU runtime.",
            },
        },
        preferences: makePreferences({compute_preference: "cpu"}),
    });
    renderEngineCard(container, entry, controllerStub, undefined);
    const feedback = container.querySelector(".le-feedback");
    assert.ok(feedback.textContent.includes("Keep the current CPU profile"), feedback.textContent);
    assert.equal(container.querySelector(".le-compute-select").value, "cpu", "失败后仍保持服务端接受值");
});

await test("compute 保存快速连续切换：旧响应不覆盖新值，失败按 live state 回滚", async () => {
    const container = makeContainer();
    const catalog = makeCatalog().map((item) => item.engine_id === "funasr"
        ? {
            ...item,
    ...
    const raceController = {
        ...controllerStub,
        getState: () => new Map([["funasr", entry]]),
        savePreferences: (_engineId, patch) => new Promise((resolve, reject) => {
            pending.push({patch, resolve, reject});
        }),
    };
    renderEngineCard(container, entry, raceController, undefined);
    const config = container.querySelector(".le-card-config");
    ...
    select.value = "cpu";
    select._fire("change", {target: select});
    assert.equal(pending.length, 2, "两个请求均已发出");
    pending[1].reject(new Error("save failed"));
    pending[0].resolve({compute_preference: "vulkan"});
    await new Promise((resolve) => setTimeout(resolve, 0));
    assert.equal(select.value, "cpu", "旧成功响应不得覆盖最新失败的服务端值");
    assert.equal(select.disabled, false, "保存完成后控件恢复可用");
    assert.ok(config.textContent.includes("保存失败"), "失败时显示可行动回滚文案");
});

await test("组件卸载后迟到 compute 响应不写回已移除控件", async () => {
    const container = makeContainer();
    const catalog = makeCatalog().map((item) => item.engine_id === "funasr"
        ? {
            ...item,
    ...
    let resolveSave;
    const unloadController = {
        ...controllerStub,
        savePreferences: () => new Promise((resolve) => { resolveSave = resolve; }),
    };
    renderEngineCard(container, entry, unloadController, undefined);
    const config = container.querySelector(".le-card-config");
    const select = config.querySelector(".le-compute-select");
    select.value = "vulkan";
    select._fire("change", {target: select});
    config.textContent = "";
    resolveSave({compute_preference: "vulkan"});
    await new Promise((resolve) => setTimeout(resolve, 0));
    assert.equal(select.dataset.savedValue, "cpu", "迟到响应不得更新已卸载控件的 saved value");
});
