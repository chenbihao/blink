...
function updateServerRowStatus(name, status, toolCount, errorMsg, tools) {
  ...
  if (status === "probing") {
    ...
    // offline
    if (dot) { dot.className = "mcp-status-dot mcp-dot-offline"; }
    if (toolInfo) { toolInfo.textContent = "0 个工具"; }
    if (errEl && errorMsg) {
...
async function handleImportLoad() {
  ...
  if (selectAllLoading) { selectAllLoading.disabled = true; selectAllLoading.checked = true; }
